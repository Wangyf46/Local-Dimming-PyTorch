...
def getLD_interpolation(BL_init, gray):
    H, W = gray.shape
    h, w = BL_init.shape
    m, n = H/h, W/w
    ...
    for i in range(h):
        ...
        for j in range(w):
            y1 = int(n * j)
            y2 = int(n * (j + 1))
            LD[x1:x2, y1:y2] = BL_init[i][j]
    return LD


def getLD_bma(BL_init, gray):
    H, W = gray.shape
    ## TODO
    ...
    LD = cv2.resize(BL_blur_2x, (W, H))
    LD = np.where(LD < 0.0, 0.0, LD)
    LD = np.where(LD > 255.0, 255.0, LD)
    return LD
